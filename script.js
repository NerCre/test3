...
(() => {
  ...
  function initOnePage() {
    ...
    // 事故区分定義（ユーザー指定の順序・内容）
    const accidentDefs = [
      { key: 'fall', label: '転落', iconFile: 'icon_acc_fall.png' },
      { key: 'crush', label: '挟まれ', iconFile: 'icon_acc_crush.png' },
      { key: 'flying', label: '飛来', iconFile: 'icon_acc_struck.png' },
      { key: 'collapse', label: '倒壊', iconFile: 'icon_acc_collapse.png' },
      { key: 'burn', label: '熱傷', iconFile: 'icon_acc_burn.png' },
      { key: 'hazard', label: '有害物', iconFile: 'icon_acc_harmful.png' },
      { key: 'electric', label: '感電', iconFile: 'icon_acc_electric.png' },
      { key: 'collision', label: '激突', iconFile: 'icon_acc_collision.png' },
      { key: 'explosion', label: '爆発', iconFile: 'icon_acc_explosion.png' },
      { key: 'other', label: 'その他', iconFile: 'icon_acc_other.png' },
    ];
    ...
    if (iconWrap && iconWrap.children.length === 0) {
      accidentDefs.forEach((def) => {
        ...
        div.innerHTML = `<div class="icon"><img src="${def.iconFile}" alt="" aria-hidden="true"></div><div class="label">${def.label}</div>`;
